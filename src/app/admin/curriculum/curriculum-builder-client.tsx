...
type Curriculum = { id: string; academic_session_id: string; term_id: string; class_id: string; subject_id: string; title: string; description: string | null; status: "draft" | "published" | "archived"; week_count: number };
...
type ApiResult = CurriculumBuilderData & { error?: string };
...
export default function CurriculumBuilderClient({ initialData }: { initialData: CurriculumBuilderData }) {
  ...
  const missingSubjects = useMemo(() => data.subjects.filter((item) => !existingSubjectIds.has(item.id)), [data.subjects, existingSubjectIds]);
  const recommendedSubjects = useMemo(() => {
    const sectionSubjects = SUBJECT_CATALOG.filter((item) => item.sections.includes(section) || section === "Other");
    const recommendedNames = new Set(sectionSubjects.map((item) => item.name.toLowerCase()));
  ...
  const [activeSubjectId, setActiveSubjectId] = useState(existingPlans[0]?.subject_id ?? "");
  const [weeks, setWeeks] = useState<{ id?: string; weekNumber: number; title: string; summary: string }[]>([]);
  ...
  const activeUnit = activePlan ? data.units.find((unit) => unit.curriculum_id === activePlan.id && unit.title.toLowerCase() === "weekly plan") ?? data.units.find((unit) => unit.curriculum_id === activePlan.id) : undefined;
  const activeTopics = activePlan ? data.topics.filter((topic) => topic.unit_id === activeUnit?.id).filter((topic) => topic.week_number !== null).sort((a, b) => (a.week_number ?? 0) - (b.week_number ?? 0)) : [];

  function hydrateWeeks(plan: Curriculum | undefined) {
    if (!plan) {
      setWeeks([]);
      return;
    }
    const planTopics = data.topics
      .filter((topic) => data.units.some((unit) => unit.id === topic.unit_id && unit.curriculum_id === plan.id))
      .filter((topic) => topic.week_number !== null)
      .sort((a, b) => (a.week_number ?? 0) - (b.week_number ?? 0));
    const count = plan.week_count || 10;
    setWeekCount(count);
    setWeeks(Array.from({ length: count }, (_, index) => {
      const number = index + 1;
      const topic = planTopics.find((item) => item.week_number === number);
      return { id: topic?.id, weekNumber: number, title: topic?.title === `Week ${number}` ? "" : topic?.title ?? "", summary: topic?.summary ?? "" };
    }));
  }
  ...
  function changeClass(nextClassId: string) {
    ...
    setActiveSubjectId(nextPlan?.subject_id ?? "");
    hydrateWeeks(nextPlan);
    setSelectedSubjectIds([]);
  }
  ...
  async function post(action: string, payload: Record<string, unknown>) {
    ...
    setMessage("");
    try {
      const response = await fetch("/api/admin/curriculum", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, schoolId: data.school.id, ...payload }) });
  ...
  async function createSelectedCurricula() {
    ...
    setActiveSubjectId(nextPlan?.subject_id ?? "");
    setMessage(`${selectedSubjectIds.length} subject plan${selectedSubjectIds.length === 1 ? "" : "s"} ready.`);
    hydrateWeeks(nextPlan);
  }
  ...
  async function addCustomSubject() {
    ...
    setNewCustomSubject("");
    setSelectedSubjectIds((current) => [...current, created.id]);
    setMessage(`${created.name} added to your school subjects.`);
  }
  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        <div><p className="eyebrow">CURRICULUM</p><h1>Build the term once.</h1><p className="muted">Pick the class context, tick the subjects, choose the number of weeks, then fill each subject week by week.</p></div>
        <div className={styles.context}><span>{data.school.code}</span><strong>{currentSession?.name ?? "No session"}</strong><span>{currentTerm?.name ?? "No term"}</span></div>
      </header>
      ...
      <section className={styles.contextCard}>
        <div className={styles.sectionTop}><div><strong>1. Curriculum context</strong><span>Session is fixed to the current school session.</span></div><span className={styles.currentBadge}>{currentSession?.name ?? "Not set"}</span></div>
      ...
      <section className={styles.subjectCard}>
        <div className={styles.sectionTop}><div><strong>2. Subjects</strong><span>Existing curricula appear automatically. Only missing subjects need to be selected.</span></div><span className={styles.count}>{existingPlans.length} existing</span></div>
        {existingPlans.length ? <div className={styles.existingList}>{existingPlans.map((plan) => <button type="button" key={plan.id} className={`${styles.subjectTab} ${activeSubjectId === plan.subject_id ? styles.active : ""}`} onClick={() => { setActiveSubjectId(plan.subject_id); hydrateWeeks(plan); }}><span>{subjectName(plan.subject_id)}</span><small>{plan.week_count} weeks · {plan.status}</small></button>)}</div> : <div className={styles.empty}>No curriculum exists for this class yet.</div>}
      ...
      {activePlan && <section className={styles.editorCard}>
        ...
        <div className={styles.subjectSwitcher}>{existingPlans.map((plan) => <button type="button" key={plan.id} className={activePlan.id === plan.id ? styles.switchActive : styles.switch} onClick={() => { setActiveSubjectId(plan.subject_id); hydrateWeeks(plan); }}>{subjectName(plan.subject_id)}</button>)}</div>
      ...
      {!activePlan && <section className={styles.emptyEditor}><strong>Choose subjects above to start.</strong><span>Any curriculum already created for this class will appear here automatically.</span></section>}
    </div>
  );
}
