...
type Result=CurriculumBuilderData&{error?:string;subject?:Subject};
...
export default function CurriculumBuilderV2({initialData}:{initialData:CurriculumBuilderData}){
 ...
 async function post(action:string,payload:Record<string,unknown>){setBusy(true);setError("");setMessage("");try{const r=await fetch("/api/admin/curriculum",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action,schoolId:data.school.id,...payload})});const out=await r.json() as Result;if(!r.ok){setError(out.error||"Unable to update curriculum.");return null}setData(out);return out}catch{setError("We couldn't reach the curriculum service. Try again.");return null}finally{setBusy(false)}}
 async function createCurriculum(){if(!selected.length)return;const r=await post("create_curricula",{sessionId,termId,classId,subjectIds:selected});if(!r)return;const ps=r.curricula.filter(c=>c.academic_session_id===sessionId&&c.term_id===termId&&c.class_id===classId);const p=ps.find(c=>selected.includes(c.subject_id))||ps[0];setSelected([]);if(p)openPlan(p,r);setMessage("Curriculum created. Configure each subject separately.")}
 ...
 async function save(){if(!active)return;const r=await post("save_weeks",{curriculumId:active.id,weekCount,topicsPerWeek,weeks:topicDrafts});if(!r)return;const p=r.curricula.find(c=>c.id===active.id);if(p)openPlan(p,r);setMessage(`${name(active.subject_id)} curriculum saved.`)}
