...
function formatDate(value: string) {
  return new Intl.DateTimeFormat("en-NG", { day: "numeric", month: "short", year: "numeric" }).format(new Date(`${value.slice(0, 10)}T00:00:00`));
}
...
export default function CurriculumClient({ initialData }: { initialData: CurriculumData }) {
  ...
  const [form, setForm] = useState<CurriculumForm>({
    ...
    title: "Primary 5 Mathematics · First Term",
    description: "Curriculum sequence for classroom teaching, lesson preparation and assessment planning.",
  });
  ...
  function relatedName(kind: "session" | "term" | "class" | "subject", id: string) {
    const item = kind === "session" ? data.sessions.find((value) => value.id === id) : kind === "term" ? data.terms.find((value) => value.id === id) : kind === "class" ? data.classes.find((value) => value.id === id) : data.subjects.find((value) => value.id === id);
  ...
  async function submit(action: string, body: Record<string, unknown>, success: string) {
    ...
    try {
      ...
      const result = (await response.json()) as CurriculumData & { error?: string };
      if (!response.ok) {
  ...
  async function saveCurriculum(event: FormEvent<HTMLFormElement>) {
    ...
    const ok = await submit(form.id ? "update_curriculum" : "create_curriculum", { curriculumId: form.id, sessionId: form.sessionId, termId: form.termId, classId: form.classId, subjectId: form.subjectId, title: form.title, description: form.description }, form.id ? "Curriculum updated." : "Curriculum created as a draft.");
    if (ok && !form.id) {
      const match = data.curricula.find((item) => item.title === form.title);
  ...
  async function saveUnit(event: FormEvent<HTMLFormElement>) {
    ...
    const ok = await submit(unitForm.id ? "update_unit" : "create_unit", { curriculumId: selectedId, unitId: unitForm.id, unitNumber: unitForm.unitNumber, unitTitle: unitForm.title, unitSummary: unitForm.summary, sortOrder: unitForm.unitNumber }, unitForm.id ? "Unit updated." : "Unit added.");
    if (ok) {
      setUnitForm({ unitNumber: selectedUnits.length + 1, title: "", summary: "" });
      setSelectedUnitId("");
    }
  }
  ...
  async function saveTopic(event: FormEvent<HTMLFormElement>) {
    ...
    const ok = await submit(topicForm.id ? "update_topic" : "create_topic", { unitId: selectedUnitId, topicId: topicForm.id, topicTitle: topicForm.title, topicSummary: topicForm.summary, weekNumber: topicForm.weekNumber ? Number(topicForm.weekNumber) : null, lessonCount: topicForm.lessonCount, sortOrder: data.topics.filter((topic) => topic.unit_id === selectedUnitId).length + 1 }, topicForm.id ? "Topic updated." : "Topic added.");
    if (ok) setTopicForm({ title: "", summary: "", weekNumber: "", lessonCount: 1 });
  }

  async function changeStatus(action: "publish_curriculum" | "archive_curriculum") {
    if (!selectedId) return;
    const ok = await submit(action, { curriculumId: selectedId }, action === "publish_curriculum" ? "Curriculum published." : "Curriculum archived.");
    if (ok && action === "archive_curriculum") setSelectedId("");
  }
  ...
  return (
    <>
      <section className="page-heading">
        <div>
          ...
          <div className="context-strip">
            ...
            <span className="context-chip">Session <strong>{relatedName("session", form.sessionId)}</strong></span>
          </div>
        </div>
      ...
      <section className="surface curriculum-context-card">
        <div className="section-heading"><div><h2>Curriculum context</h2><p>One curriculum owns a specific academic session, term, class and subject.</p></div><span className="status status-provisioning">Draft-first</span></div>
        <form className="curriculum-form" onSubmit={saveCurriculum}>
          <div className="form-grid">
            <label className="field">Academic session<select value={form.sessionId} onChange={(event) => { const next = event.target.value; const nextTerm = data.terms.find((term) => term.academic_session_id === next && term.is_current) ?? data.terms.find((term) => term.academic_session_id === next); setForm({ ...form, sessionId: next, termId: nextTerm?.id ?? "" }); }} required><option value="">Select session</option>{data.sessions.map((session) => <option value={session.id} key={session.id}>{session.name}{session.is_current ? " (Current)" : ""}</option>)}</select></label>
            <label className="field">Term<select value={form.termId} onChange={(event) => setForm({ ...form, termId: event.target.value })} required><option value="">Select term</option>{availableTerms.map((term) => <option value={term.id} key={term.id}>{term.name}</option>)}</select></label>
            <label className="field">Class<select value={form.classId} onChange={(event) => setForm({ ...form, classId: event.target.value })} required><option value="">Select class</option>{data.classes.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label>
            <label className="field">Subject<select value={form.subjectId} onChange={(event) => setForm({ ...form, subjectId: event.target.value })} required><option value="">Select subject</option>{data.subjects.map((item) => <option value={item.id} key={item.id}>{item.name}{item.code ? ` (${item.code})` : ""}</option>)}</select></label>
          </div>
          ...
          <div className="form-footer"><p>Keep the title human-readable. This becomes the teaching plan name in the curriculum library.</p><button className="btn btn-primary" type="submit" disabled={saving}>{saving ? "Saving…" : form.id ? "Save curriculum" : "Create curriculum"}</button></div>
        </form>
      </section>

      <section className="curriculum-workbench">
        <aside className="surface curriculum-list-card">
          <div className="section-heading"><div><h2>Your curricula</h2><p>{data.curricula.length} curriculum plan{data.curricula.length === 1 ? "" : "s"}</p></div><span className="count-badge">{data.curricula.length}</span></div>
          <div className="curriculum-list">
            {data.curricula.length ? data.curricula.map((item) => {
              const active = selectedId === item.id;
              const className = relatedName("class", item.class_id);
              const subjectName = relatedName("subject", item.subject_id);
              const termName = relatedName("term", item.term_id);
              return <button type="button" className={`curriculum-item ${active ? "is-selected" : ""}`} key={item.id} onClick={() => { setSelectedId(item.id); setForm({ id: item.id, sessionId: item.academic_session_id, termId: item.term_id, classId: item.class_id, subjectId: item.subject_id, title: item.title, description: item.description ?? "" }); setUnitForm({ unitNumber: data.units.filter((unit) => unit.curriculum_id === item.id).length + 1, title: "", summary: "" }); }}><span className="curriculum-item-title">{item.title}</span><span>{className} · {subjectName}</span><small>{termName} · <strong>{item.status}</strong></small></button>;
            }) : <div className="empty-state"><strong>No curriculum plans yet.</strong><p>Create your first class/subject plan above.</p></div>}
          </div>
        </aside>

        <section className="surface curriculum-editor">
          {!selected ? (
            <div className="editor-empty"><div className="editor-mark">C</div><p className="eyebrow">CURRICULUM EDITOR</p><h2>Select a curriculum to build its sequence.</h2><p className="muted">Units and topics will appear here. Keep the curriculum in draft while you are shaping the teaching sequence, then publish it when it is ready for teachers.</p></div>
          ) : (
            <>
              <div className="editor-header">
                <div><p className="eyebrow">{selected.status.toUpperCase()}</p><h2>{selected.title}</h2><p className="muted">{relatedName("class", selected.class_id)} · {relatedName("subject", selected.subject_id)} · {relatedName("term", selected.term_id)}</p></div>
                <div className="row-actions"><span className={`status status-${selected.status === "published" ? "accepted" : selected.status === "archived" ? "revoked" : "provisioning"}`}>{selected.status}</span>{selected.status === "draft" && <button className="btn btn-primary" type="button" onClick={() => void changeStatus("publish_curriculum")} disabled={saving}>Publish</button>}{selected.status !== "archived" && <button className="button-danger" type="button" onClick={() => void changeStatus("archive_curriculum")} disabled={saving}>Archive</button>}</div>
              </div>
      ...
      <style jsx global>{`
        ...
        .unit-create .form-grid { grid-template-columns: 120px minmax(0,1fr); }
